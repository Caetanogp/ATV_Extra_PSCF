...
1 = bloco dividido (possui dois “buddies”)

2 = bloco usado (alocado para um programa)

char[] programaNo
Guarda o rótulo do programa associado a um nó quando ele está USADO.

int[] tamanhoProgKB
Guarda o tamanho real do programa (em KB) para exibição no relatório.

Índices dos nós:

Raiz da árvore: índice 0 → representa o bloco de 4 MB.
...
A alocação é feita por uma função recursiva (alocarRec) que percorre a árvore:

Se o nó está livre e o bloco tem tamanho exato do pedido, retorna aquele índice.

Se o nó está livre e o bloco é maior que o necessário, esse nó é marcado como DIVIDIDO e o bloco é repartido em dois “buddies” (filho esquerdo e direito).
...
Se os dois filhos do mesmo pai estão LIVRE e o pai está DIVIDIDO, os dois são fundidos em um bloco maior e o pai passa a ser LIVRE.

Esse processo continua enquanto for possível, restaurando blocos maiores e reduzindo a fragmentação.

(No teste principal, a liberação pode ser chamada manualmente a partir do main se quiser demonstrar no vídeo.)
